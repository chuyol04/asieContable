...
export default async function MyPayrollPage({ searchParams }: PageProps<"/mis-nominas">) {
  ...
  return <div className="min-h-screen bg-[#f4f7fb] text-[#17233c]">
    <header className="border-b border-slate-200 bg-white"><div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6"><div className="flex min-w-0 items-center gap-3"><BrandLogo className="h-10 w-28" /><div className="min-w-0 border-l border-slate-200 pl-3"><p className="truncate text-xs text-slate-400">Portal de cliente</p><p className="truncate text-sm font-semibold text-slate-800">{client?.name ?? "Cuenta sin asociación"}</p></div></div><form action={logoutAction}><button className="rounded-lg border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-600" type="submit">Salir</button></form></div></header>
    <main className="mx-auto max-w-6xl px-4 py-7 sm:px-6 sm:py-10"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-cyan-700">Consulta documental</p><h1 className="mt-2 text-2xl font-semibold tracking-tight text-slate-950 sm:text-3xl">Mis nóminas</h1><p className="mt-2 text-sm text-slate-500">Consulta y descarga tus archivos privados usando tu sesión de PHANTO.</p>
