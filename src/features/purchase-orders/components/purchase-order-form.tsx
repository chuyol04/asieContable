...
type EditableLineField = "unit" | "quantity" | "unitPrice" | "discount" | "taxRate";
...
export function PurchaseOrderForm({ action, cancelHref, companyName, initialLines = [], initialValues, orderNumber, products, submitLabel, supplierCreateHref, suppliers }: { action: PurchaseOrderFormAction; cancelHref: string; companyName: string; initialLines?: OrderEditorLine[]; initialValues: OrderEditorValues; orderNumber: string; products: Product[]; submitLabel: string; supplierCreateHref: string; suppliers: Supplier[] }) {
  ...
  function updateLine(key: string, field: EditableLineField, value: string) { setLines((current) => current.map((line) => line.key === key ? { ...line, [field]: value } : line)); }
  ...
  return <form action={formAction} className="space-y-6">
    <input name="items" type="hidden" value={JSON.stringify(lines.map(({ itemId, productId, unit, quantity, unitPrice, discount, taxRate }) => ({ itemId, productId, unit, quantity, unitPrice, discount, taxRate })))} />
...
function EditableCells({ line, onChange }: { line: OrderEditorLine; onChange: (key: string, field: EditableLineField, value: string) => void }) { return <><td className="px-3 py-4"><SmallInput inputMode="text" label="Unidad" maxLength={64} value={line.unit} onChange={(value) => onChange(line.key, "unit", value)} /></td><td className="px-3 py-4"><SmallInput label="Precio unitario" value={line.unitPrice} onChange={(value) => onChange(line.key, "unitPrice", value)} /></td><td className="px-3 py-4"><SmallInput label="Cantidad" value={line.quantity} onChange={(value) => onChange(line.key, "quantity", value)} /></td><td className="px-3 py-4"><SmallInput label="Descuento" value={line.discount} onChange={(value) => onChange(line.key, "discount", value)} /></td><td className="px-3 py-4"><SmallInput label="IVA" value={line.taxRate} onChange={(value) => onChange(line.key, "taxRate", value)} /></td></>; }
function SmallInput({ inputMode = "decimal", label, maxLength, onChange, value }: { inputMode?: "decimal" | "text"; label: string; maxLength?: number; onChange: (value: string) => void; value: string }) { return <input aria-label={label} className="w-28 rounded-lg border border-slate-300 px-3 py-2 text-sm" inputMode={inputMode} maxLength={maxLength} onChange={(event) => onChange(event.target.value)} value={value} />; }
function MobileField({ inputMode = "decimal", label, maxLength, onChange, value }: { inputMode?: "decimal" | "text"; label: string; maxLength?: number; onChange: (value: string) => void; value: string }) { return <label className="text-xs font-semibold text-slate-500">{label}<input className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900" inputMode={inputMode} maxLength={maxLength} onChange={(event) => onChange(event.target.value)} value={value} /></label>; }
