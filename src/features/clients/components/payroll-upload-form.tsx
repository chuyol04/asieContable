...
import type { ClientPayrollCompany, PayrollFormAction, PayrollFormState } from "../types";
...
export function PayrollUploadForm({ action, companies, initialMonth, initialYear }: { action: PayrollFormAction; companies: ClientPayrollCompany[]; initialMonth: number; initialYear: number }) {
  ...
  return <form action={formAction} className="space-y-5">
    ...
    <div className="grid gap-5 sm:grid-cols-2">
      <label className="text-xs font-semibold text-slate-600 sm:col-span-2">Empresa de nómina *<select className={inputClass} defaultValue="" name="payrollCompanyId" required><option disabled value="">Selecciona una empresa</option>{companies.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</select></label>
