...
export function PayrollUploadForm({ action, initialMonth, initialYear }: { action: PayrollFormAction; initialMonth: number; initialYear: number }) {
  ...
  return <form action={formAction} className="space-y-5">
    ...
    <div className="grid gap-5 sm:grid-cols-2">
      ...
      <label className="text-xs font-semibold text-slate-600">Archivo *<input accept=".pdf,.xls,.xlsx,application/pdf,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" className={inputClass} name="payrollFile" required type="file" /><span className="mt-1 block font-normal text-slate-400">PDF, XLS o XLSX; máximo 20 MB.</span></label>
    </div>
    <label className="block text-xs font-semibold text-slate-600">Observaciones<textarea className={inputClass} maxLength={5_000} name="notes" rows={3} /></label>
    <div className="flex justify-end"><button className="rounded-lg bg-cyan-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60" disabled={pending} type="submit">{pending ? "Subiendo…" : "Subir nómina"}</button></div>
  </form>;
}
