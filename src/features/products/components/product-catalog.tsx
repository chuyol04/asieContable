...
export function ProductCatalog({ companyId, products, search, standalone = false, status }: { companyId: number; products: Product[]; search: string; standalone?: boolean; status: ProductStatusFilter }) {
  return <>
    <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-lg font-semibold text-slate-900">Productos</h2><p className="mt-1 text-sm text-slate-500">Catálogo propio de la empresa para futuras órdenes de compra.</p></div><div className="flex flex-col gap-2 sm:flex-row"><Link className="rounded-lg border border-cyan-600 bg-white px-4 py-2.5 text-center text-sm font-semibold text-cyan-700 hover:bg-cyan-50" href="/productos/importar">Cargar Excel</Link><Link className="rounded-lg bg-cyan-600 px-4 py-2.5 text-center text-sm font-semibold text-white hover:bg-cyan-700" href={`/empresas/${companyId}/productos/nuevo`}>Nuevo producto</Link></div></div>
